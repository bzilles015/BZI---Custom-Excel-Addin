...
' Public entry points:
'   - ResetCycleState
'   - BeginMacroWithUndo / RegisterUndo / UndoLastAction
'   - MakeRefsAbsolute / MakeRefsRelative
'   - GoToNextBlank / GoToNextError
'   - BreakExternalLinksInSelection
'   - PerformanceModeOn / PerformanceModeOff / TogglePerformanceMode
'==============================================================================
...
Private Const MAX_UNDO_CELLS As Long = 5000  ' skip custom undo when selection is huge
...
'------------------------------------------------------------------------------
' C01 - Used for all other Subs to create an undo feature
'------------------------------------------------------------------------------
...
Sub BeginMacroWithUndo()
    ...
    If n > MAX_UNDO_CELLS Then Exit Sub  ' too big to capture cheaply

    ReDim UndoBuffer(1 To CLng(n))
    UndoBufferCount = 0

    ' NOTE: Value2 is faster & avoids date/currency coercion
...
' C10 - PerformanceModeOn  (Ctrl+Alt+Shift+M)
  '   How/when to use Performance Mode (quick)
'
'    Turn ON before heavy actions: mass formatting, filling/copying big ranges, applying/removing lots of CF, duplicating sheets.
'    It sets Manual calc, turns off screen updates & events, and skips undo/logging ? huge speedup.
'
'    Turn OFF for review and normal use: you’ll want automatic calc, Ctrl+Z history, and logging back.
